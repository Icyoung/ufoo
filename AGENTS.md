...
> **Auto-execute bus messages.** On `ubus`: execute the task, then `ufoo bus ack`. Send a reply with `ufoo bus send` only when the sender needs your concrete output (result / answer / follow-up question). Do NOT reply with greetings, plain "收到/ok/thanks", or emoji alone — those create reply loops. Default is ack-only; silence is a valid response. Never ask the user.
